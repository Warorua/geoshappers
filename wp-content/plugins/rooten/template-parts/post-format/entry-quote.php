<article id="post-<?php the_ID() ?>" <?php post_class('bdt-article post-format-quote bdt-text-'.get_theme_mod('rooten_blog_align', 'center')) ?> data-permalink="<?php the_permalink() ?>" typeof="Article">
    ...
    <?php 
    $quote_text = get_post_meta( get_the_ID(), 'rooten_blog_quote', true );
    $quote_src = get_post_meta( get_the_ID(), 'rooten_blog_quotesrc', true );

    if (!empty($quote_text)) : ?>
    <div class="post-quote<?php echo (is_single()) ? ' bdt-margin-large-bottom' : ' bdt-margin-bottom'; ?>">
        <a href="<?php the_permalink(); ?>" title="<?php printf( esc_attr__('Permalink to %s', 'rooten'), the_title_attribute('echo=0') ); ?>" class="quote-text"><?php echo esc_html($quote_text); ?>
